...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event.is_pressed() and (event is InputEventKey or event is InputEventJoypadButton):
		var action_event_list = InputMap.action_get_events(action)
		if action_event_index < action_event_list.size():
			InputMap.action_erase_event(action, action_event_list[action_event_index])
